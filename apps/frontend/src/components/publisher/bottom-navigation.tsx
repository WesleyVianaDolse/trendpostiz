...
function HomeIcon() {
  return (
    <svg aria-hidden="true" viewBox="0 0 24 24" className="h-5 w-5" fill="none">
      <path
        d="M3.5 10.8 12 3.5l8.5 7.3v8.7a1 1 0 0 1-1 1h-5v-6h-5v6h-5a1 1 0 0 1-1-1v-8.7Z"
        stroke="currentColor"
        strokeWidth="1.8"
    ...
    >
      <div className="flex">
        ...
        <NavigationItem
          ...
          label="Criar"
          icon={<PlusIcon />}
          active={pathname.startsWith('/publish/new')}
        />
        <NavigationItem
          href="/publish/scheduled"
          label="Agenda"
          icon={<CalendarIcon />}
          active={pathname.startsWith('/publish/scheduled')}
        />
        <NavigationItem
          href="/publish/profile"
          label="Perfil"
          icon={<ProfileIcon />}
          active={pathname.startsWith('/publish/profile')}
        />
      </div>
    </nav>
  );
